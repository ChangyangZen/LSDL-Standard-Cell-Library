...
# PATCHED: original file had nmos/pmos gate lengths swapped. GF180 PL.2 requires
# NMOS Lmin=0.6um, PMOS Lmin=0.5um (5V/6V) — matches nfet_05v0/pfet_05v0 and our
# hand-source lsdl_*.spice. The swap drew a DRC-illegal 0.5um NMOS and broke LVS.
gate_length_nmos = 280*nm if targetvoltage=='3.3V' else 600*nm if targetvoltage=='5V' else 700*nm # PL.2 https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_08.html
gate_length_pmos = 280*nm if targetvoltage=='3.3V' else 500*nm if targetvoltage=='5V' else 550*nm # PL.2 https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_08.html
...
unit_cell_width = 3*560*nm # PATCHED 2->3 tracks: widen gate pitch so field poly is >=0.3um (PL.5_MV) from neighbor S/D COMP. At 2 tracks the gap was only 0.08um (failed PL.5). # measured from gf180mcu_fd_sc_mcu9t5v0__inv_1 -> 1.12 um = 2 Tracks = 2*0.56 nm
...
via_size = {
    l_poly_contact: 220*nm, # PATCHED 230->220: CO.1 is exact-size (without_length(0.22um)); GDS contacts must be exactly 0.22um. https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_13.html
    l_ndiff_contact: 220*nm, # PATCHED 230->220: CO.1 exact 0.22um https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_13.html
    l_pdiff_contact: 220*nm, # PATCHED 230->220: CO.1 exact 0.22um https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_13.html
...
minimum_enclosure = {
    ...
    (l_metal1, l_pdiff_contact): 60*nm, # PATCHED 40->60: CO.6a end-of-line (M1<0.34um) overlap of contact needs 0.06um https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_13.html
    (l_metal1, l_ndiff_contact): 60*nm, # PATCHED 40->60: CO.6a end-of-line overlap https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_13.html
    (l_metal1, l_poly_contact): 60*nm, # PATCHED 40->60: CO.6a end-of-line overlap https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_13.html
    # The minimum is 5nm, but since we want to do the overlap symmetrical to achieve reproducibility, when we use 40nm we evade the rule that we would have to use 60nm on the other side:
    (l_metal1, l_via1): 70*nm, # PATCHED 40->70: via1 landing pad = 0.26+2*0.07=0.40um -> 0.16um^2 >= Mn.3 0.1444 (was 0.34um=0.1156, failed M1.3) https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_15.html
    (l_metal2, l_via1): 70*nm, # PATCHED 40->70: via1 landing pad >= Mn.3 min area (was failing M2.3) https://gf180mcu-pdk.readthedocs.io/en/latest/physical_verification/design_manual/drm_07_15.html
